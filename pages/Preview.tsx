...
export const Preview = () => {
  const { profile, incomes, expenses } = useUser();
  ...
  // 売上合計
  const totalIncomeValue = incomes.reduce((sum, item) => sum + (item.amount || 0), 0);
  const incomeTotal = totalIncomeValue.toLocaleString();

  // 経費内訳の計算関数
  const getExpenseTotalByCategory = (categoryName: string) => {
    return expenses
      .filter(item => item.category === categoryName)
      .reduce((sum, item) => sum + (item.amount || 0), 0);
  };

  // 各カテゴリごとの合計 (表示用フォーマット済)
  const expenseMap = {
    '給料賃金': getExpenseTotalByCategory('給料賃金').toLocaleString(),
    '外注工賃': getExpenseTotalByCategory('外注工賃').toLocaleString(),
    '減価償却費': getExpenseTotalByCategory('減価償却費').toLocaleString(), // 要減価償却入力からの連携
    '地代家賃': getExpenseTotalByCategory('地代家賃').toLocaleString(),
    '利子割引料': getExpenseTotalByCategory('利子割引料').toLocaleString(),
    '租税公課': getExpenseTotalByCategory('租税公課').toLocaleString(),
    '荷造運賃': getExpenseTotalByCategory('荷造運賃').toLocaleString(),
    '水道光熱費': getExpenseTotalByCategory('水道光熱費').toLocaleString(),
    '旅費交通費': getExpenseTotalByCategory('旅費交通費').toLocaleString(),
    '通信費': getExpenseTotalByCategory('通信費').toLocaleString(),
    '広告宣伝費': getExpenseTotalByCategory('広告宣伝費').toLocaleString(),
    '接待交際費': getExpenseTotalByCategory('接待交際費').toLocaleString(),
    '損害保険料': getExpenseTotalByCategory('損害保険料').toLocaleString(),
    '修繕費': getExpenseTotalByCategory('修繕費').toLocaleString(),
    '消耗品費': getExpenseTotalByCategory('消耗品費').toLocaleString(),
    '福利厚生費': getExpenseTotalByCategory('福利厚生費').toLocaleString(),
    '雑費': getExpenseTotalByCategory('雑費').toLocaleString(),
  };

  const totalExpenseValue = expenses.reduce((sum, item) => sum + (item.amount || 0), 0);
  const expensesTotal = totalExpenseValue.toLocaleString();

  // 専従者・貸倒等はデフォルト0として、事業所得(利益)の計算
  const profitValue = Math.max(0, totalIncomeValue - totalExpenseValue);
  const profit = profitValue.toLocaleString();
          ...
          >
            ...
            <div className="flex border border-black text-[9px]" style={{ height: TABLE_HEIGHT }}>
              ...
              <div className="w-1/3 border-r border-black flex h-full">
                ...
                <div className="flex-1 flex flex-col">
                  <GridRow label="給料賃金" id="⑪" val={expenseMap['給料賃金'] !== '0' ? expenseMap['給料賃金'] : ""} />
                  <GridRow label="外注工賃" id="⑫" val={expenseMap['外注工賃'] !== '0' ? expenseMap['外注工賃'] : ""} />
                  <GridRow label="減価償却費" id="⑬" val={expenseMap['減価償却費'] !== '0' ? expenseMap['減価償却費'] : ""} />
                  <GridRow label="貸倒金" id="⑭" val="" />
                  <GridRow label="地代家賃" id="⑮" val={expenseMap['地代家賃'] !== '0' ? expenseMap['地代家賃'] : ""} />
                  <GridRow label="利子割引料" id="⑯" val={expenseMap['利子割引料'] !== '0' ? expenseMap['利子割引料'] : ""} />
                  <GridRow label="租税公課" id="㋑" val={expenseMap['租税公課'] !== '0' ? expenseMap['租税公課'] : ""} />
                  <GridRow label="荷造運賃" id="㋺" val={expenseMap['荷造運賃'] !== '0' ? expenseMap['荷造運賃'] : ""} />
                  <GridRow label="水道光熱費" id="㋩" val={expenseMap['水道光熱費'] !== '0' ? expenseMap['水道光熱費'] : ""} />
                  <GridRow label="旅費交通費" id="㋥" val={expenseMap['旅費交通費'] !== '0' ? expenseMap['旅費交通費'] : ""} />
                  <GridRow label="通信費" id="㋭" val={expenseMap['通信費'] !== '0' ? expenseMap['通信費'] : ""} />
                  <GridRow label="広告宣伝費" id="㋬" val={expenseMap['広告宣伝費'] !== '0' ? expenseMap['広告宣伝費'] : ""} />
                  <GridRow label="接待交際費" id="㋣" val={expenseMap['接待交際費'] !== '0' ? expenseMap['接待交際費'] : ""} />
                  <GridRow label="損害保険料" id="㋠" val={expenseMap['損害保険料'] !== '0' ? expenseMap['損害保険料'] : ""} />
                  <GridRow label="修繕費" id="リ" val={expenseMap['修繕費'] !== '0' ? expenseMap['修繕費'] : ""} />
                  <GridRow label="消耗品費" id="ヌ" val={expenseMap['消耗品費'] !== '0' ? expenseMap['消耗品費'] : ""} />
              ...
              <div className="w-1/3 flex h-full">
                ...
                <div className="flex-1 flex flex-col">
                  <GridRow label="福利厚生費" id="ル" val={expenseMap['福利厚生費'] !== '0' ? expenseMap['福利厚生費'] : ""} />
                  <GridRow label="雑費" id="ヲ" val={expenseMap['雑費'] !== '0' ? expenseMap['雑費'] : ""} />
